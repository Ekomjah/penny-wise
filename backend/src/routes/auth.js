...
router.post("/register", registerLimiter, async (req, res) => {
  try {
    ...
    if (authorInviteCode) {
      ...
      if (!process.env.AUTHOR_SECRET_CODE || process.env.AUTHOR_SECRET_CODE.length < 16) {
        return res.status(500).json({ error: "Server misconfiguration: Author registration is currently disabled." });
      }
